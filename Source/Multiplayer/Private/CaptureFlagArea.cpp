...
void ACaptureFlagArea::CanIncreseCapture()
{
	if (!bCaptured && !bContesting)
	{
		if (RedTeamPlayersArray.Num() == 0 && BlueTeamPlayersArray.Num() == 0)
		{
			float DecrementTime = CurrentCaptureTime - LosingSpeed;
			SetCurrentCaptureTime(DecrementTime);
		}
		else
		{
			if (RedTeamPlayersArray.Num() > 0 && BlueTeamPlayersArray.Num() == 0)
			{
				...
				{
					float Capture = CurrentCaptureTime + CapturingSpeed;
					SetCurrentCaptureTime(Capture);
				}
			}
			else if (BlueTeamPlayersArray.Num() > 0 && RedTeamPlayersArray.Num() == 0)
			{
				if (CurrentTeam != ETeam::ET_BlueTeam)
				{
					float DecrementTime = CurrentCaptureTime -	LosingSpeed;
					SetCurrentCaptureTime(DecrementTime);
